<template>
        ...
        <q-card style="min-width: 400px">
          ...
          <q-card-section>
            <div v-for="field in createFields" :key="field.field" class="q-my-sm">
              <DynamicMasterForm
                v-model="createFormData[field.field]"
                :field="field"
              />
            </div>
          </q-card-section>
  ...
  function buildOption(item) {
    const label =
      typeof props.field === "function" ? props.field(item) : item[props.field];
    return {
      label,
      value: item,
    };
  ...
  /**
   * Carga inicial de datos, y si hay defaultIds, los trae
   */
  function fetchDefaultData() {
    ...
    api
      .get(urlWithParam)
      .then((response) => {
        const defaultValues = response.data.results.map((item) =>
          buildOption(item)
        );
        model.value = props.multiple ? defaultValues : defaultValues[0] || null;
      })
      .finally(() => {
        isLoading.value = false;
      });
